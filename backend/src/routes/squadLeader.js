...
// POST /api/squad-leader/create-scout
router.post('/create-scout', async (req, res) => {
  const {
    username, password, name, firstName, lastName, prefix, nickname,
    birthDate, school, province, phone, email, allergies, congenitalDisease
  } = req.body

  const existingUser = await prisma.user.findUnique({ where: { username } })
  if (existingUser) throw createError(400, 'ชื่อผู้ใช้นี้มีอยู่แล้วในระบบ')
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const squadLeader = await tx.user.findUnique({
      where: { id: req.user.id },
      select: { school: true }
    })
    
    if (squadLeader.school && school !== squadLeader.school) {
      throw createError(400, `ลูกเสือต้องมาจากสถานศึกษาเดียวกับผู้กำกับหมู่ (${squadLeader.school})`)
    }
    ...
    const scout = await tx.scout.create({
      data: {
        ...
        congenitalDisease,
        squadId: squad.id,
        scoutCode: `SC${Date.now().toString().slice(-6)}`
      }
    })
  ...
  await logAudit({
    userId: req.user.id,
    action: 'CREATE_SCOUT_IN_SQUAD',
    target: result.scout.id,
    after: { squadId: squad.id }
  })

  res.json(result.scout)
})
