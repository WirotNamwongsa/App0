...
export default function LeaderSchedule() {
  const { data: me } = useQuery('leader-me', () => api.get('/auth/me'))
  const leadingSquad = me?.leadingSquad
  const campId = leadingSquad?.troop?.campId || leadingSquad?.troop?.camp?.id
  const squadId = leadingSquad?.id

  const { data: schedules = [], isLoading } = useQuery(
    ['leader-schedule', campId],
  ...
  return (
    <div className="page">
      <PageHeader title="ตารางกิจกรรมหมู่ของฉัน" />
      ...
      {isLoading ? (
        ...
        <div className="card text-center py-10 text-gray-400">
          ...
          <p>ยังไม่มีตารางกิจกรรมสำหรับหมู่ของฉัน</p>
        </div>
      ) : (
