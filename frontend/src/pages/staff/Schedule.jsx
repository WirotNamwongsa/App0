...
export default function StaffSchedule() {
  ...
  // เช็คว่ามีข้อมูล schedules และไม่ใช่ loading
  ...
  const handleViewParticipants = (scheduleId) => {
    navigate(`/staff/schedule/${scheduleId}/participants`)
  }
                ...
                return (
                  <div key={slot.value} className="card">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs font-semibold text-scout-700">{slot.label}</p>
                      <button
                        onClick={() => handleViewParticipants(items[0].id)}
