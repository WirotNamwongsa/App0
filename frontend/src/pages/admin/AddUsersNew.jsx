...
const EMPTY = {
  ...
  campId: '', squadId: '', citizenId: '', gender: '',
}
...
export default function AdminAddUsers() {
  ...
  const createMutation = useMutation(d => api.post('/admin/accounts', d), {
    onSuccess: () => { setForm(EMPTY); setErrors({}); setShowModal(false); toast.success('สร้างบัญชีสำเร็จ') },
    onError: (error) => { toast.error(error.response?.data?.error || 'สร้างบัญชีไม่สำเร็จ') }
  })
  ...
  function validate() {
    ...
    if (!form.campId && form.role !== 'ADMIN') e.campId = 'กรุณาเลือกค่าย'
  ...
  function submit() {
    ...
    const userData = {
      ...
      firstName: form.firstName, lastName: form.lastName, prefix: form.prefix,
    }
    ...
    if (form.role === 'SCOUT') {
      Object.assign(userData, {
        ...
        province: form.province, school: form.school,
        phone: form.phone, email: form.email,
        allergies: form.allergies, congenitalDisease: form.congenitalDisease,
        squadId: form.squadId,
      })
    } else if (form.role === 'TROOP_LEADER') {
      Object.assign(userData, { nickname: form.nickname, school: form.school, phone: form.phone, squadId: form.squadId })
    } else if (form.role === 'CAMP_MANAGER') {
      Object.assign(userData, { email: form.email, phone: form.phone })
    } else if (form.role === 'STAFF') {
      Object.assign(userData, { school: form.school, phone: form.phone })
    } else if (form.role === 'ADMIN') {
    ...
    <div className="page">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        {['ADMIN', 'CAMP_MANAGER', 'TROOP_LEADER', 'STAFF'].map(role => {
          ...
          return (
            <button key={role} onClick={() => { set('role', role); setShowModal(true) }}
      ...
      <div className="mb-8">
        <button onClick={() => { set('role', 'SCOUT'); setShowModal(true) }}
        ...
        <Modal onClose={() => { setShowModal(false); setForm(EMPTY); setErrors({}) }}>
          <div className="px-5 pt-5 pb-4 flex items-center justify-between border-b border-gray-100 dark:border-scout-700">
            <div className="flex items-center gap-3">
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${isScout ? 'bg-emerald-100 dark:bg-emerald-900/40' : 'bg-green-100 dark:bg-green-900/50'}`}>
                {isScout ? <Users size={18} className="text-emerald-600 dark:text-emerald-400" /> : <Shield size={18} className="text-green-800 dark:text-green-400" />}
              </div>
              <div>
                <h3 className="text-base font-bold text-gray-900 dark:text-white">เพิ่ม{roleConfig[form.role]?.label}</h3>
          ...
          <div className="px-5 pt-4">
            <div className="flex bg-gray-100 dark:bg-scout-900 p-1 rounded-xl overflow-x-auto gap-1">
              {[
                { v: 'ADMIN', l: 'ผู้ดูแลระบบ' }, { v: 'CAMP_MANAGER', l: 'ผู้ดูแลค่าย' },
                { v: 'TROOP_LEADER', l: 'ผู้กำกับหมู่' }, { v: 'STAFF', l: 'ผู้จัดกิจกรรม' },
                { v: 'SCOUT', l: 'ลูกเสือ' },
              ].map(r => (
                <button key={r.v} onClick={() => { set('role', r.v); setErrors({}) }}
          ...
          <div className="px-5 py-4 space-y-4">
            ...
            {isScout ? (
              <Field label="เลขบัตรประชาชน 13 หลัก * (ใช้เป็นรหัสผ่าน)" error={errors.citizenId}>
                <input type="tel" inputMode="numeric" value={form.citizenId}
                  onChange={e => {
                    const val = e.target.value.replace(/\D/g, '')
                    set('citizenId', val)
                    if (val.length > 0 && val.length < 13) setErrors(er => ({ ...er, citizenId: `กรอกแล้ว ${val.length}/13 หลัก` }))
                    else setErrors(er => ({ ...er, citizenId: '' }))
                  }}
                  className={INPUT} placeholder="กรอกเลขบัตรประชาชน 13 หลัก" maxLength={13} />
                <p className="text-xs mt-1 text-gray-400">{form.citizenId.length}/13 หลัก · 🔒 ใช้เป็นรหัสผ่านในการเข้าสู่ระบบ</p>
              </Field>
            ) : (
            ...
            <Field label="คำนำหน้า *" error={errors.prefix}>
              <select value={form.prefix} onChange={e => set('prefix', e.target.value)} className={INPUT}>
                ...
                {(isScout ? ['ด.ช.', 'ด.ญ.', 'นาย', 'นาง', 'นางสาว'] : ['นาย', 'นาง', 'นางสาว']).map(p => <option key={p} value={p}>{p}</option>)}
              </select>
            </Field>
          ...
          <div className="px-5 pb-6 flex gap-3">
            ...
            <button onClick={submit} disabled={createMutation.isLoading}
