...
import { BottomNav, PageHeader, Card, Spinner, Button } from '../../components/common/UI';
...
export default function CampStructure() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #0f2440, #0a1628)', paddingBottom: 80 }}>
        ...
        {troops.map(troop => (
          <Card key={troop.id}>
            ...
            {troop.patrols.map(patrol => (
              <button key={patrol.id} onClick={() => navigate(`/camp/patrol/${patrol.id}`)} style={{
                width: '100%', display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8, padding: '10px 12px', marginBottom: 6, color: 'var(--text)',
              }}>
      ...
      <BottomNav items={NAV} active="structure" onSelect={handleNav} />
    </div>
  );
}
