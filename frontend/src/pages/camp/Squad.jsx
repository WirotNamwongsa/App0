...
export default function CampSquad() {
  const { id } = useParams();
  ...
  useEffect(() => {
    load();
    campApi.getStructure().then(r => setAllSquads(r.data.flatMap(t => t.squads?.map(s => ({ ...s, troop: t })) || [])));
  }, [id]);

  const load = () => campApi.getSquad(id).then(r => setSquad(r.data)).catch(console.error).finally(() => setLoading(false));
  ...
  const handleAdd = async () => {
    try {
      await campApi.addScout({ ...addForm, squadId: id });
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #0f2440, #0a1628)', paddingBottom: 40 }}>
      <PageHeader
        title={`${squad?.name} — หมู่ ${squad?.number}`}
        subtitle={`${squad?.scouts?.length || 0} คน · ${squad?.leader?.displayName || 'ยังไม่มีผู้กำกับ'}`}
        onBack={() => navigate(-1)}
      ...
      <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {squad?.scouts?.map(scout => (
          <Card key={scout.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ flex: 1 }}>
              <p 
                style={{ fontWeight: 600, fontSize: '0.9rem', cursor: 'pointer', color: '#1e40af', textDecoration: 'underline' }}
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  console.log('Navigating to scout profile:', scout.id, scout.firstName, scout.lastName);
                  navigate(`/scout/profile/${scout.id}`);
                }}
              >
        ...
        {squad?.scouts?.length === 0 && <div style={{ textAlign: 'center', padding: 40, color: '#475569' }}><p style={{ fontSize: '2rem' }}>👥</p><p style={{ marginTop: 8 }}>ยังไม่มีสมาชิก</p></div>}
      </div>
      ...
      {showMove && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 200, display: 'flex', alignItems: 'flex-end' }}>
          <div style={{ background: 'var(--bg-card)', borderRadius: '20px 20px 0 0', padding: 24, width: '100%' }}>
            ...
            <p style={{ color: '#64748b', fontSize: '0.85rem', marginBottom: 16 }}>จาก {squad?.name}</p>
            <select value={moveTarget} onChange={e => setMoveTarget(e.target.value)} style={{ marginBottom: 16 }}>
              ...
              {allSquads.filter(s => s.id !== id).map(s => <option key={s.id} value={s.id}>หมู่ {s.number} {s.name}</option>)}
            </select>
      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 200, display: 'flex', alignItems: 'flex-end', overflow: 'auto' }}>
          <div style={{ background: 'var(--bg-card)', borderRadius: '20px 20px 0 0', padding: 24, width: '100%', maxHeight: '90vh', overflow: 'auto' }}>
            <h2 style={{ fontWeight: 700, marginBottom: 16 }}>เพิ่มลูกเสือใหมู่</h2>
            {[
              ['firstName', 'ชื่อ'],
              ['lastName', 'สกุล'],
              ['school', 'โรงเรียน'],
